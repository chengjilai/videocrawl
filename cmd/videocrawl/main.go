...
func main() {
	...
	case "enumerate":
		...
		fs.Parse(reorderArgs(args, map[string]bool{}))
		err = a.Enumerate(*concurrency, *limit, *source)
	case "download":
		fs := flag.NewFlagSet("download", flag.ExitOnError)
		limit := fs.Int("limit", 0, "max videos this pass (0=all queued)")
		workers := fs.Int("workers", 3, "parallel downloads")
		...
		err = a.Download(*limit, *workers, policy)
	case "crawl-loop":
		...
		err = a.CrawlLoop(*every, *limit, *workers, *rounds, policy)
	case "status":
		err = a.Status()
	case "list":
...
func usage() {
  ...
  add <kind> <url-or-id> [--name N] [--query Q]
      kinds: youtube-channel youtube-playlist bilibili-space
...
examples:
  ...
  videocrawl crawl-loop --every 3600 --limit 10 --workers 6`)
}
